...
export default function ({ permissions, setPermissions }: Props) {
  ...
  useEffect(() => {
    setFiltered(data.filter((permission: IPermission) => {
      if (!permissions.map((item: IPermission) => item.id).includes(permission.id)) return permission
    }))
  }, [permissions])
